...
class GBA_algorithms:
    ...
    def plot_gradient_ascent_trajectory( self ):
        ...
        # Plot 1: mu vs Time
        plt.subplot(2, 2, 1)
        plt.plot(self.t_trajectory, self.mu_trajectory, color='b', label='mu(t)', linestyle='-', marker='o')
        plt.xlabel('Time')
        plt.ylabel('mu')
        ...
        plt.subplot(2, 2, 2)
        plt.plot(self.dt_trajectory, color='r', label='dt', linestyle='-', marker='x')
        plt.yscale('log')
        ...
        plt.subplot(2, 2, 3)
        plt.plot(self.dmu_trajectory, color='g', label='mu_diff', linestyle='-', marker='s')
        plt.yscale('log')
        plt.xlabel('Iteration')
    ...
    def plot_MCMC_trajectory( self, path = None ):
        ...
        plt.title('Model ' + self.model_name+ ' fluxfraction rate over time with highlighted mutations', fontsize = 14)
        plt.legend(loc='center left', bbox_to_anchor=(1, 0.5), fontsize=10)
        plt.grid(True)
        plt.subplot(2, 1, 2)
        ...
        plt.title('Model ' + self.model_name+ 'condition' +self.condition+ 'Mu trajectory over time', fontsize=14)
